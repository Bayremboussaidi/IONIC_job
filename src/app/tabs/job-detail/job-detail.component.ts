...
export class JobDetailPage implements OnInit {
  ...
  ngOnInit() {
    ...
    const job = this.jobs.find(x => x.id.toString() === id); // Ensure to compare with string
    if(job) {
      this.job = job;
    }
  }
  ...
  changeSegment(event: any) {
    console.log(event);
    this.segment_value = event.detail.value; // Adjust according to your event structure
  }
